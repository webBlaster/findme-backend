/**
 * Example client for the Geolocation Broadcast System (Production)
 * This example shows how to connect to a remote server (e.g., on fly.io)
 *
 * Usage:
 *   - For local: deno run --allow-net examples/client-remote.ts
 *   - For fly.io: SERVER_URL="https://yourapp.fly.dev" deno run --allow-net examples/client-remote.ts
 */
...
function connectToRoom(roomId: string): WebSocket {
  // Get the WS URL from the same host as our API
  const isSecure = SERVER_URL.startsWith("https");
  const protocol = isSecure ? "wss:" : "ws:";
  const hostOnly = SERVER_URL.replace(/^https?:\/\//, "");
  const wsUrl = `${protocol}//${hostOnly}/ws/${roomId}`;
